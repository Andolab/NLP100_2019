...
def expect_label(vector: list, W: list, stand_prob: float):
    probs = []
    expects = []
    for vec in vector:
...
if __name__ == "__main__":
    ...
    rec_list = np.array([])
    for k in range(10):
        (expects, probs) = expect_label(v_test, W, 0.1*k)
        corrects = [0 if int(lab) != 1 else 1 for lab in l_test.tolist()]
        expects = [0 if int(lab) != 1 else 1 for lab in expects]
        pre_list = np.append(pre_list, precision_score(corrects, expects))
        rec_list = np.append(rec_list, recall_score(corrects, expects))
    print(k)
    print(pre_list)
    print(rec_list)
    draw_figure(pre_list, rec_list)
