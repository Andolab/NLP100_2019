...
def pick_word(W: list, word_list: list):
    ...
    word_num = 10
    unimp_word = [word_dict[val] for val in sort_value[:word_num]]
    imp_word = [word_dict[val] for val in sort_value[-1-word_num:-1:]]
...
if __name__ == "__main__":
    ...
    (imp_word, unimp_word) = pick_word(W, adj_list)
    print("  高素性  :  低素性  (重みの降順)")
    for imp, unimp in zip(imp_word, unimp_word):
        print("{0}:{1}".format(imp.center(10), unimp.center(10)))
